...
mod cursor_tests {
    ...
    use crate::{
        mm::{
            frame::FrameMeta, page::allocator, CachePolicy, PageFlags, PageProperty, PageTable,
            PagingConsts,
        },
        prelude::*,
    };
    ...
    fn test_cursor_move_forward() {
        ...
        let mut cursor = Cursor::new(&pt, &range).unwrap();
        let page_size = page_size::<PagingConsts>(cursor.level);
        let next_va = cursor.va.align_down(page_size) + page_size;
        cursor.move_forward();
        assert_eq!(cursor.virt_addr(), next_va);
    }
