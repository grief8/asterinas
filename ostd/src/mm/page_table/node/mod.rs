...
mod tests {
    use super::*;
    use crate::{
        mm::{page_table::UserMode, CachePolicy, PageFlags, PageProperty, PageTable, PAGE_SIZE},
        prelude::*,
    };

    // #[ktest]
    // fn test_raw_page_table_node_activate() {
    //     // Test that `RawPageTableNode::activate` correctly activates the page table.
    //     let pt = PageTable::<UserMode>::empty();
    //     let raw_node = pt.root.clone_shallow();
    //     unsafe { raw_node.activate(); }
    //     // Verify that the page table is activated.
    // }
