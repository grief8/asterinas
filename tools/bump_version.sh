...
print_help() {
    ...
    echo ""
    echo "Commands:"
    echo "  --docker_version_file [major|minor|patch|date]   Update Docker image version in DOCKER_IMAGE_VERSION"
    echo "  --docker_version_refs                            Update all Docker version references (except VERSION)"
    echo "  --version_file                                   Synchronize project version to Docker version (update VERSION)"
