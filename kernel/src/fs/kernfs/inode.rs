...
impl PseudoNode for KernfsNode {
    ...
    fn set_data(&self, data: Box<dyn DataProvider>) -> Result<()> {
        debug!("[yyy] set_data: name = {:?}", self.name());
...
impl Inode for KernfsNode {
    ...
    fn resize(&self, new_size: usize) -> Result<()> {
        debug!("[yyy] resize: new_size = {}", new_size);
    ...
    fn write_at(&self, offset: usize, buf: &mut VmReader) -> Result<usize> {
        debug!("[yyy] write_at: offset = {}", offset);
    ...
    fn rmdir(&self, name: &str) -> Result<()> {
        ...
        debug!("rmdir4: name = {:?}", name);
        self.remove(name)?;
        debug!("rmdir5: name = {:?}", name);
        Ok(())
    }
    ...
    fn write_link(&self, target: &str) -> Result<()> {
        ...
        debug!("[yyy] write_link: target = {:?}", target);
