...
static KERNEL_KOBJECT: Once<Arc<KObject>> = Once::new();
static DEVICES_KOBJECT: Once<Arc<KObject>> = Once::new();
...
impl PseudoFileSystem for SysFS {
    ...
    fn init(&self) -> Result<()> {
        ...
        let kernel_obj = SysFS::create_kobject("kernel", 0o755, root.clone())?;
        KERNEL_KOBJECT.call_once(|| kernel_obj);
        init_kernel()?;
        let devices_obj = SysFS::create_kobject("devices", 0o755, root.clone())?;
        DEVICES_KOBJECT.call_once(|| devices_obj);
        init_devices()?;
        Ok(())
    }
